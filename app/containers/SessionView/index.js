...
export default class SessionView extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  render() {
    ...
    return (<div className={styles.sessionView}>
      {this.canEdit() ? <PublishHeader h2={session && session.state === 'published' ? 'Published activity' : 'Preview'} actions={this.getActions()} /> : null}
      <NotificationBar />
      {session ? this.renderSession() : <LoadingMessage message="activity not found" />}
    </div>);
  }
}
