      ...
      users.forEach(user => {
        ...
        if (analysis.sessions.published.length) {
          if (analysis.totals.expire.past.week && !analysis.totals.expire.future) {
            ...
            <p>Thanks for being one of the first providers to use the LeisureDesc uploader. However, we’ve noticed you’ve been a little inactive lately.</p>
            <p>Login <a href="https://app.opensessions.io/">here</a> and upload more sessions on LeisureDesc to ensure they are are visible to thousands of people on Get Active every month.</p>
            ...
            email = ['Good news from LeisureDesc', user.email, `<p>Dear ${user.given_name || user.name},</p>
            <p>Thanks for being one of the first providers to use the LeisureDesc uploader. We wanted to let you know that your session information is now <b>live</b> on activity finders like Get Active!</p>
            <p>If your session information is up to date, please simply reply YES to this email. Otherwise, please visit LeisureDesc and update them.</p>
